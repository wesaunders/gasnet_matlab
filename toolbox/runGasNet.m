...
%                                       = fitFunction(output,t,ouputNodes)
...
function [output,fitness] = runGasNet(genotype,elecThresh,gasThresh,...
                            globalC,globalK,nomValues,display,maxT,...
                            fitFunction,fitLevel,getInput,outputNodes,...
                            modulationFunction, diffusionFunction)
if(nargin<14)
...
for geneIndex=1:noOfGenes
    ...
    for valueIndex=1:valueLength
        if(valueIndex == 3 || valueIndex == 4 || valueIndex == 6 ||...
                                                        valueIndex == 7)
...
% Stores the output of all nodes at each time step.
output = cell(1,maxT-1);
output{1} = zeros(1,noOfGenes);

%The fitness cell array.
% Stores the fitness for each timestep.
fitness = cell(1,maxT-1);
fitness{1} = 0;


if(display), fig1 = figure; end
t = 2;
while(t < maxT && fitness{t-1} < fitLevel)
